...
// Captain / First Officer: two IDENTITIES, so two categorical slots. They were
// briefly a two-step burgundy pair because the monthly-intake chart stacked
// them into one column; that chart is gone, and the pair had a real cost –
// ROLE_FO was byte-identical to STAGE_RAMP[0], so registering its dark twin
// overwrote the stage ramp's and the ordinal ramp inverted in dark mode.
// Taking documented slots means the dark steps come from the scale itself and
// no second mapping can collide with anything.
export const ROLE_CPT = CATEGORICAL[0]
export const ROLE_FO = CATEGORICAL[3]

// ORE priority is ORDINAL (A before B before C), so one hue getting darker.
export const ORE_COLORS = { A: STAGE_RAMP[4], B: STAGE_RAMP[2], C: STAGE_RAMP[0] }
// Aircraft are identities.
export const AC_COLORS = { A320: CATEGORICAL[0], B737: CATEGORICAL[1] }
