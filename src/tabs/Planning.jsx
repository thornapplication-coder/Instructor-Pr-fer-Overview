...
export default function Planning({ view: viewProp, embedded }) {
  ...
  const rows = useMemo(() => {
    ...
    return trainers
      ...
      //
      // Through isOwnStaff, not isInternal: since 1.47.0 a qualification can
      // say "extern" in its own name (TRE extern / TRI extern), and this was
      // the one place still asking only about the affiliation - so somebody on
      // an external GRADE was still bookable onto a course here while every
      // other view had already let them go.
      .filter(isOwnStaff)
